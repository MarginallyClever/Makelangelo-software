...
//		} else {
			...
			y = (b-k) / ik;
		//}
        cc.setRGB(px, py, encode32bit(255-(int)(c*255.0)));
        cm.setRGB(px, py, encode32bit(255-(int)(m*255.0)));
        cy.setRGB(px, py, encode32bit(255-(int)(y*255.0)));
        ck.setRGB(px, py, encode32bit(255-(int)(k*255.0)));
      }
    }
  ...
  public TransformedImage processViaHistogram(TransformedImage img) {
    ...
    for (y = 0; y < h; ++y) {
      for (x = 0; x < w; ++x) {
        ...
        img.getSourceImage().setRGB(x, y, encode32bit(b));
      }
    }
