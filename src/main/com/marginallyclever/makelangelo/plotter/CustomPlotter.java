...
public class CustomPlotter extends Makelangelo3 {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3182996965899090146L;
	...
	public Point2D getHome() {
		return new Point2D(0,getLimitBottom());
	}
...
//		paintCalibrationPoint(gl2);
		paintControlBox(gl2);
		paintMotors(gl2);
		paintPenHolderToCounterweights(gl2);
		paintSafeArea(gl2);
	}
	 ...
	 * @param settings
	 */
	protected void paintControlBox(GL2 gl2) {
		double cy = getLimitTop();
	...
	protected void paintPenHolderToCounterweights( GL2 gl2 ) {
		double dx,dy;
		double gx = getPenX();// / 10;
		double gy = getPenY();// / 10;
	...
	protected void paintSafeArea(GL2 gl2) {
    ...
    public String getGCodeConfig() {
		String result = super.getGCodeConfig();
		double beltlen = Math.sqrt(
						Math.pow(getHomeX()-getLimitLeft(),2)+
						Math.pow(getLimitTop()-getLimitBottom(),2)
						);
