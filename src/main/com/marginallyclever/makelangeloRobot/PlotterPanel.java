 ...
 * @since 7.1.4
 */
public class PlotterPanel extends JPanel implements PlotterEventListener {
	/**
	 *
	 */
	 ...
	 * @param myRobot
	 */
	public PlotterPanel(Plotter robot) {
		super();
	...
	private JPanel createConnectSubPanel() {
        ...
        buttonConnect.addPropertyChangeListener((evt)->{
			if(mySession!=null && mySession.isOpen()) {
				...
				mySession = NetworkSessionManager.requestNewSession(me);
				if(mySession!=null) {
	...
	private void updateMachineNumberPanel() {
		...
		buttonOpenSettings.addActionListener((e)->{
			...
			JDialog dialog = new JDialog(frame, Translator.get("configureMachine"));
			dialog.add(new PlotterSettingsPanel(myRobot));
			...
			// Save the gcode so that updates to settings are applied immediately.
