...
public class FirmwareUploaderPanel extends SelectPanel {
	 ...
	 */
	private final SelectFile sourceAVRDude = new SelectFile("path", Translator.get("FirmwareUploaderPanel.avrDude path"), firmwareUploader.getAvrdudePath());
	 ...
	 *
	 * TODO ? file size check. As this is a firmware the .hex is not a bit to
	 * bit the flash size ( as for a .bin), but normaly the size of the file
	 * have to be ??? less or equal to ?2x 512Ko or ? depending of the maximum
	 * flash capacity of the microchip. ( basicaly if we got a 10 Mb file for a
	 * atmega2560 (flash size of ?512KB ) there is something wrong ... )
	 */
	private final SelectFile sourceHex = new SelectFile("file", Translator.get("FirmwareUploaderPanel.HexFile"), "");

	/**
	 * User interface to select/change the Serial COM port to use. Can be
	 * refresh. Sould be filed with available COM Port.
	 */
	private final SelectOneOfMany port = new SelectOneOfMany("port", Translator.get("FirmwareUploaderPanel.Port"));
	...
	private final ButtonIcon portRefreshJButtonIcon = new ButtonIcon("FirmwareUploaderPanel.Refresh", "/images/arrow_refresh.png");
	 ...
	 */
	private final SelectButton goButton = new SelectButton("upload", Translator.get("FirmwareUploaderPanel.Upload"));
	 ...
	 */
	private final SelectTextArea selectTextAreaForAvrdudeExecLog = new SelectTextArea("avrdude_logs", Translator.get("FirmwareUploaderPanel.avrdude.logs"), "");
	...
	/**
	 * To automaticaly populate the hex source file JTextField. ... the first of
	 * the .hex files found in the currend PWD/"." dir.
	 *
	...
	private String[] getListOfCOMPorts() {
		return new SerialTransportLayer().listConnections().toArray(String[]::new);
	}
...
//Translator.get("AboutAvrdude");
	 ...
	 *
	 * Or do i remove this message. (lets the user find it by himself with no
	 * leads )
	 *
	 * @param html
	 * @return
