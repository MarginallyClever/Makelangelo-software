...
public class Makelangelo5 implements PlotterRenderer {
	...
	public void render(ShaderProgram shader, GL3 gl, Plotter robot) {
		...
		if (textureMotors == null) {
			...
			paintControlBoxFancy(shader, gl, robot, textureMotors);
		}
	...
	private void paintGondola(GL3 gl2, double gx, double gy,Plotter robot) {
		...
		DrawingHelper.drawCircle(gl2, (float)gx, (float)gy, Polargraph.PEN_HOLDER_RADIUS_2, Color.WHITE);
		if (robot.getPenIsUp()) {
			DrawingHelper.drawCircle(gl2, (float)gx, (float)gy, Polargraph.PEN_HOLDER_RADIUS_2 + 5, Color.WHITE);
		}
	}
	...
	private void paintControlBoxFancy(ShaderProgram shader, GL3 gl, Plotter robot,TextureWithMetadata texture) {
		...
		paintTexture(shader, gl, texture, ox, oy, TW, TH);
	}
