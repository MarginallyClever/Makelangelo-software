...
public class ColorTurtle extends Node {
    ...
    public void update() {
        ...
        if( moved.strokeLayers.size() == 1 ) {
            moved.strokeLayers.getFirst().setColor(c);
        } else if( moved.strokeLayers.size() > 1 ) {
            // many layers merge into one, provided they are the same diameter
            // TODO check for diameter changes
            StrokeLayer newLayer = new StrokeLayer(c, moved.strokeLayers.getFirst().getDiameter());
            for( var layer : moved.strokeLayers ) {
                newLayer.addAll(layer.getAllLines());
            }
            moved.strokeLayers.clear();
            moved.strokeLayers.add(newLayer);
        }
